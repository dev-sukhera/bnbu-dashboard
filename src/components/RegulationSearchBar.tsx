...
interface RegulationSearchFilters {
  query: string; // Address, City, or Area
  status: string;
  startDate?: string;
  endDate?: string;
}
...
const RegulationSearchBar: React.FC<RegulationSearchBarProps> = ({ onSearch }) => {
  ...
  const handleSearch = () => {
    ...
    const formattedEndDate = endDate 
      ? new Date(endDate.getTime() + 86399999).toISOString().split('T')[0] 
      : undefined;
    onSearch({ query, status, startDate: formattedStartDate, endDate: formattedEndDate });
  };
